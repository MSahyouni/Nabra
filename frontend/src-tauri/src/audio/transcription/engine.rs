...
/// Validate that a Whisper transcription model is ready before starting recording.
pub async fn validate_transcription_model_ready<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    ...
    match config.provider.as_str() {
        ...
        other => {
            ...
            Err(format!(
                "Provider '{}' is not supported for local transcription. Please select 'localWhisper'.",
                other
            ))
        }
    }
            ...
            .await
        {
            Ok(Some(config)) => {
                ...
                if config.provider == "localWhisper" {
                    ...
                    // Non-Whisper provider - this function shouldn't be called.
                    return Err(format!(
