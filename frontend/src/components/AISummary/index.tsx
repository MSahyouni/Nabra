...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const convertToMarkdown = () => {
    let markdown = `# ملخص ذكي للاجتماع: ${meeting?.id || 'غير معروف'} - ${meeting?.title || 'اجتماع بلا عنوان'}\n\n`;
    ...
    Object.entries(currentSummary).forEach(([key, section]) => {
      if (key === 'title') {
        markdown = `# ${section.title || 'ملخص محسّن بالذكاء الاصطناعي'}\n\n`;
      } else {
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">خطأ في إنشاء الملخص</h3>
      </div>
      <p className="text-red-600 text-sm">{error}</p>
      <p className="text-red-500 text-xs mt-2">تحقق من إعداد النموذج ومفاتيح الواجهة البرمجية، أو حاول مجددًا.</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing' ? 'جارٍ معالجة التفريغ' : 'جارٍ إنشاء الملخص'}
          </h3>
          <p className="text-blue-600 text-sm">
            {status === 'processing' 
              ? 'جارٍ تحليل التفريغ...'
              : 'جارٍ إنشاء ملخص مفصل للاجتماع...'}
          </p>
        </div>
      </div>
    </div>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">لا يوجد محتوى ملخص متاح.</p>
        <p className="text-gray-500 text-sm mt-1">حاول إنشاء ملخص جديد.</p>
      </div>
    );
  }
      ...
      {/* <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            ...
            <span className="ml-1">إعادة الإنشاء</span>
          </button>
        </div>
      </div> */}
