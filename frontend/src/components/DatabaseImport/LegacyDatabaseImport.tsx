...
export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage('لم يتم العثور على قاعدة بيانات في الموقع المحدد. اختر مجلد نَبْرَة القديم أو مجلد الخادم أو ملف قاعدة البيانات مباشرة.');
  ...
  const handleStartFresh = async () => {
    try {
      ...
      setImportState('success');
      toast.success('تم تجهيز قاعدة البيانات بنجاح! جارٍ تشغيل التطبيق...');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">أهلًا بك في نَبْرَة!</DialogTitle>
          <DialogDescription className="text-base pt-2">
            هل لديك بيانات من تثبيت سابق؟
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              اختر مجلد التطبيق السابق أو مجلد الخادم أو ملف قاعدة البيانات:
            </p>
            ...
            >
              {importState === 'selecting' || importState === 'detecting' ? (
                <>
                  ...
                  <span>{importState === 'selecting' ? 'جارٍ الاختيار...' : 'جارٍ اكتشاف قاعدة البيانات...'}</span>
                </>
              ) : (
                <>
                  <FolderOpen className="h-5 w-5" />
                  <span>استعراض قاعدة البيانات</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">تم العثور على قاعدة البيانات!</p>
