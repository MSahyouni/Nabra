...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError('فشل تحميل أجهزة الصوت. تحقق من إعدادات صوت النظام.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError('لم يُعثر على ميكروفون لمراقبته');
        return;
      }
      ...
      setError('تعذّر بدء مراقبة مستوى الصوت');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">أجهزة الصوت</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="default">الميكروفون الافتراضي</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">لم يُعثر على ميكروفون</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">مستويات الميكروفون:</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="default">صوت النظام الافتراضي</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">لم يُعثر على جهاز لصوت النظام</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>الميكروفون:</strong> يسجّل صوتك والأصوات المحيطة</p>
        <p>• <strong>صوت النظام:</strong> يسجّل صوت الحاسوب، مثل الموسيقى والمكالمات</p>
        {isMonitoring && (
          <p>• <strong>مستويات الميكروفون:</strong> الأخضر جيد، والأصفر مرتفع، والأحمر مرتفع جدًا</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>نصيحة:</strong> اضغط «اختبار الميكروفون» للتحقق من عمله</p>
        )}
      </div>
    </div>
  );
