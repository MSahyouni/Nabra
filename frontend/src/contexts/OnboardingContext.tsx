...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const [summarySkipped, setSummarySkipped] = useState(false);
    ...
    try {
      ...
      if (status) {
        ...
        if (status.completed) {
          ...
          setCompleted(true);
          setTranscriptionModelDownloaded(status.model_status.transcription === 'downloaded');
  ...
  const verifyModelStatus = async (savedStatus: OnboardingStatus) => {
    ...
    // Verify Whisper transcription model exists on disk
    try {
      await invoke('whisper_init');
      transcriptionModelDownloaded = await invoke<boolean>('whisper_has_available_models');
      console.log('[OnboardingContext] Whisper verified on disk:', transcriptionModelDownloaded);
    } catch (error) {
  ...
  const skipSummaryDownload = useCallback(() => {
    setSummarySkipped(true);
  ...
  const completeOnboarding = async () => {
    try {
      ...
      setCompleted(true);
      console.log('[OnboardingContext] Onboarding completed', summarySkipped ? '(summary skipped)' : `with model: ${modelToSave}`);
  ...
  const startBackgroundDownloads = async ({
    ...
    try {
      ...
      // Start Whisper transcription model download first (speech recognition - always required)
      if (shouldStartTranscription) {
        console.log('[OnboardingContext] Starting Whisper download:', TRANSCRIPTION_MODEL);
        invoke('whisper_init')
          .then(() => invoke('whisper_download_model', { modelName: TRANSCRIPTION_MODEL }))
  ...
  const checkActiveDownloads = async () => {
    try {
      ...
      // Also check for Built-in AI downloads if possible.
